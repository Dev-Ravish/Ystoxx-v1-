...
contract NFT1155 is ERC1155, Ownable {
    ...
    uint256 itemCount = _tokenIds.current(); // Total number of items minted
    uint256 currentIndex = 0;

    MarketItem[] memory items = new MarketItem[](itemCount); // Array to hold the market items

    for (uint256 i = 0; i < itemCount; i++) {
        MarketItem storage currentItem = idToMarketItem[i + 1]; // Adjust index to i + 1

        // Check if the item is unsold (or apply any other condition you want to filter by)
        if (currentItem.creator == address(0)) {
            items[currentIndex] = currentItem; // Add unsold item to the array
            currentIndex += 1;
        }
    }

    // Return only the unsold items (filter out empty slots)
    MarketItem[] memory unsoldItems = new MarketItem[](currentIndex);
    for (uint256 i = 0; i < currentIndex; i++) {
        unsoldItems[i] = items[i];
    }

    return unsoldItems; // Return filtered array of unsold items
}
