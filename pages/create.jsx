...
export default function Create() {
	...
	const mintNft = async (e) => {
		e.preventDefault();
		//make json data
		if (!albumName || !price || !imageUrl ) return;
		...
		try {
			...
			const receipt = await tx.wait();  // Optionally, you can add the number of confirmations
			console.log('Transaction successful: ', receipt);
			setLoadingState('Done!');
			setLoader(false);
			// Router.push('/marketplace');
		} catch (error) {
			console.error('Unable to mint NFT: ', error);
			setLoader(false);
		}
	};
